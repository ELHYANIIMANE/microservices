...
public class EurikaclientApplication {
    ...
    CommandLineRunner initializeDatabase(ClientRepo clientRepository) {
        return args -> {
            clientRepository.save(new Client(Long.parseLong("1"), "imane", Float.parseFloat("25")));
            clientRepository.save(new Client(Long.parseLong("2"), "elhyani", Float.parseFloat("24")));
            clientRepository.save(new Client(Long.parseLong("3"), "soula", Float.parseFloat("23")));
        };
    }
}
